Xray SS2022 + Reality 一键管理脚本
[](https://img.shields.io/badge/license-MIT-blue.svg LICENSE
[](https://img.shields.io/badge/platform-Linux-blue.svg
[](https://img.shields.io/badge/protocol-SS2022%2BReality-orange.svg
[](https://img.shields.io/badge/init-systemd%20%7C%20openrc-purple.svg
基于 大表哥 singbox-lite https://github.com/0xdabiaoge/singbox-lite 脚本二次开发
✨ 功能特性
•  🔐 SS2022 + Reality 协议 - 新一代安全代理协议，抗封锁能力强
•  🐧 多发行版支持 - 支持 Alpine (OpenRC)、Debian/Ubuntu (systemd)、CentOS/Rocky/Fedora (systemd) 等
•  🌐 双栈 IP 支持 - 支持 IPv4/IPv6 优先级设置，自动检测公网 IP
•  🛠️ 完整节点管理 - 添加、删除、修改端口、查看节点信息
•  📱 Quantumult X 配置 - 自动生成 Quantumult X 兼容配置格式
•  🔄 脚本自更新 - 一键更新脚本到最新版本
•  🧹 完整卸载 - 支持保留配置卸载或完全清理（包括脚本自毁）
📋 系统要求
项目	要求
操作系统	Alpine / Debian / Ubuntu / CentOS / Rocky / Fedora 等 Linux 发行版
包管理器	`apk` / `apt-get` / `yum` / `dnf` 任一即可
初始化系统	systemd 或 OpenRC
架构	x86_64 / AMD64 / ARM64 / ARMv7
权限	Root 用户权限
网络	可访问 GitHub 以下载 Xray 核心
🚀 快速开始
一键安装脚本
(curl -LfsS https://raw.githubusercontent.com/Ezrea7/SS2022-Reality/refs/heads/main/xray.sh -o /usr/local/bin/SS2022 || wget -q https://raw.githubusercontent.com/Ezrea7/SS2022-Reality/refs/heads/main/xray.sh -O /usr/local/bin/SS2022) && chmod +x /usr/local/bin/SS2022 && SS2022

快捷指令
安装完成后，使用以下命令进入管理菜单：
SS2022

📖 使用说明
管理菜单功能
选项	功能	说明
1	安装/更新 Xray	自动下载最新版 Xray-core
2-6	服务控制	启动/停止/重启/查看状态/查看日志
7	添加节点	创建 SS2022+Reality 节点
8	查看节点	显示所有节点信息和 Quantumult X 配置
9	删除节点	删除指定节点
10	修改端口	更改节点监听端口
11	更新脚本	从 GitHub 拉取最新版脚本
12	网络优先级	设置 IPv4/IPv6 优先
88	卸载 Xray	保留脚本，仅卸载 Xray 核心
99	卸载脚本	完全清理（包含脚本自毁）
添加节点流程
1.  运行 SS2022 进入菜单
2.  选择 [7] 添加 SS2022+Reality 节点
3.  输入服务器 IP（自动检测，可手动修改）
4.  输入监听端口（自动检测占用）
5.  输入伪装域名 SNI（默认：www.amd.com）
6.  输入节点名称（自动生成）
7.  完成！自动生成 Quantumult X 配置
⚙️ 配置文件
核心配置文件
/usr/local/etc/xray/config.json

节点元数据
/usr/local/etc/xray/metadata.json

IP 优先级配置
/usr/local/etc/xray/ip_preference.conf

日志文件
•  systemd: journalctl -u xray
•  OpenRC: 默认不保存日志（减少 IO 占用）
🔌 Quantumult X 配置示例
脚本自动生成如下格式配置：
shadowsocks=服务器IP:端口, method=2022-blake3-aes-128-gcm, password=密码, obfs=over-tls, obfs-host=伪装域名, tls-verification=true, reality-base64-pubkey=公钥, reality-hex-shortid=短ID, udp-relay=true, udp-over-tcp=sp.v2, tag=节点名称

推荐客户端: Quantumult X https://apps.apple.com/us/app/quantumult-x/id1443988620 (iOS)
🛠️ 技术细节
支持的系统初始化
•  systemd: 创建 /etc/systemd/system/xray.service
•  OpenRC: 创建 /etc/init.d/xray
Xray 版本管理
•  自动检测系统架构 (amd64/arm64/armv7)
•  从 XTLS/Xray-core https://github.com/XTLS/Xray-core 官方仓库下载
•  支持 geoip.dat / geosite.dat 自动更新
📝 更新日志
v3.5.1
•  新增 IPv4/IPv6 优先级设置功能
•  改进服务管理器兼容性
v3.5.0
•  初始版本发布
•  支持 SS2022 + Reality 单协议
•  支持 Alpine、Debian、Ubuntu、CentOS、Rocky、Fedora 等系统
🤝 致谢
•  基于 大表哥 singbox-lite https://github.com/0xdabiaoge/singbox-lite 脚本二次开发
•  XTLS/Xray-core https://github.com/XTLS/Xray-core 提供核心支持
📜 免责声明
本项目仅供学习交流使用，请遵守当地法律法规。使用本脚本搭建的服务器请勿用于非法用途。
----
Made with ❤️ by Ezrea7 https://github.com/Ezrea7

